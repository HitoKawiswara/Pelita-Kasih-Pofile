<x-app-layout>
    ...
    <div class="bg-primary-500 py-20">
        ...
        <div class="flex mx-32 text-white">
            ...
            @foreach ($struktur as $row)
                @if ($row['work_as'] == 'kepala sekolah')
                    <img src="{{ asset('storage/images/guru/' . $row['img']) }}" alt="image" class="rounded-lg h-64 aspect-video object-cover mr-10 ml-20">
                @endif
            @endforeach
        </div>
    </div>
    ...
    <div class=" mt-10">
        ...
        @foreach ($struktur as $row)
            @if ($row['work_as'] == 'kepala sekolah')
                <div class="my-card-container relative overflow-hidden">
                    ...
                    <div class="w-64 rounded-lg bg-white shadow-lg my-10 mx-5 md:mx-2 overflow-hidden my-card">
                        <img src="{{ asset('storage/images/guru/' . $row['img']) }}" alt="" class="max-h-60 w-full object-cover">
                        <ul class="text-center my-5">
                            <li class="text-2xl leading-10 font-semibold cursor-pointer">{{ ucwords(strtolower($row['name'])) }}</li>
            ...
            @elseif ($row['work_as'] == 'wakil kepala sekolah')
            <div class="my-card-container relative overflow-hidden">
                <div class="image-overlay"></div>
                <div class="w-64 rounded-lg bg-white shadow-lg my-10 mx-5 md:mx-2 overflow-hidden my-card">
                    <img src="{{ asset('storage/images/guru/' . $row['img']) }}" alt="" class="max-h-60 w-full object-cover">
        ...
        <div class="grid grid-cols-5">
            @foreach ($struktur as $row)
                @if ($row['work_as'] != 'kepala sekolah' && $row['work_as'] != 'wakil kepala sekolah' && $row['work_as'] != 'staff')
                    <div class="flex justify-center">
                        <div class="my-card-container relative overflow-hidden">
                            <div class="image-overlay"></div>
                            <div class="w-64 rounded-lg bg-white shadow-lg my-10 mx-5 md:mx-2 overflow-hidden my-card">
                                <img src="{{ asset('storage/images/guru/' . $row['img']) }}" alt="image" class="max-h-60 w-full object-cover">
                                <ul class="text-center my-5">
