<x-admin-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @endif
                    <form action="{{ route('store_news') }}" method="POST" enctype="multipart/form-data" class="w-full p-4">
                        @csrf
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            <label for="content" class="block font-semibold mb-1">Kontent Berita (gunakan 'spasi' untuk membuat paragraf baru)</label>
    ...
    <div class="pb-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="{{ count($news) === 0  ? 'hidden' : '' }} bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                            ...
                            <div class="flex">
                                @if ($item->trashed())
                                    ...
                                    <form action="{{ route('force_delete_news', $item->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="py-2 px-4 bg-red-500 text-white font-semibold rounded-md hover:bg-red-600 focus:outline-none focus:bg-red-600">Destroy</button>
                                    </form>
                                @else
                                    ...
                                    <button onclick="editNews({{ json_encode($item->id) }}, {{ json_encode($item->title) }}, {{ json_encode(str_replace(["\r\n", "\r", "\n"], '\\n',
                                        addslashes($item->content))) }}, {{ json_encode($item->duration) }})" class="my-3 mx-3 px-4 bg-blue-500 text-white font-semibold rounded-md
                                        hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Edit</button>
                                @endif
                            </div>
                        </div>
                        {{-- edit modal start --}}
                        <div id="editModal"
                            ...
                            <div class="bg-white rounded-md p-6 w-96 md:w-3/5 lg:w-2/3">
                                ...
                                <form id="editForm">
                                    ...
                                    <div class="mb-4">
                                        <label for="editTitle" class="block text-sm font-medium text-gray-700">Judul Berita</label>
                                        <input type="text" id="editTitle"
                                            class="w-full py-2 px-3 border border-gray-300 rounded-md focus:outline-none focus:border-secondary-500" />
                                    </div>
                                    <div class="mb-4">
                                        <label for="editContent" class="block text-sm font-medium text-gray-700">Kontent Berita (gunakan 'spasi' untuk membuat paragraf baru)
                                            Berita</label>
                                    ...
                                    <div class="mb-4">
                                        <label for="editDuration"
                                            class="block text-sm font-medium text-gray-700">Durasi (hari)
                                            :</label>
