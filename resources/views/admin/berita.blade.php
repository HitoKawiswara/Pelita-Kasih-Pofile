            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="bg-white overflow-hidden shadow-lg sm:rounded-md mb-10 overflow-x-auto relative">
                        ...
                        <div class="flex">
                            @if ($item->trashed())
                            <form action="{{ route('restore_news', $item->id) }}" method="POST" class="my-3 mx-3"
                            ...
                            <form action="{{ route('force_delete_news', $item->id) }}" method="POST" class="my-3 mx-3"
                                onsubmit="handleFormSubmit(event)">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                            ...
                            @else
                            <form action="{{ route('soft_delete_news', $item->id) }}" method="POST" class="my-3 mx-3"
                                onsubmit="handleFormSubmit(event)">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                            ...
                            <button onclick="editNews('{{ $item->id }}', '{{ $item->title }}', '{{ str_replace(["
                                ...
                                class="my-3 mx-3 px-4 bg-blue-500 text-white font-semibold rounded-md hover:bg-blue-600
                                focus:outline-none focus:bg-blue-600">Edit</button>
                            @endif
                        </div>
                    </div>
                    {{-- edit modal start --}}
                    <div id="editModal"
                        ...
                        <div class="bg-white rounded-md p-6 w-96 md:w-3/5 lg:w-2/3">
                            ...
                            <form id="editForm">
                                ...
                                <div class="mb-4">
                                    <label for="editDuration" class="block text-sm font-medium text-gray-700">Durasi
                                        (hari):</label>
